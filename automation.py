...
from jobtypes.vendoring import VendorJobRunner
            ...
            for lib in libraries:
                ...
                for job in lib.jobs:
                    try:
                        ...
                        jobRunner._process_library(lib)
                    except Exception as e:
                        self.logger.log("Caught an exception while processing library %s job type %s" % (lib.name, job['type']), level=LogLevel.Error)
                        self.logger.log_exception(e)
        except Exception as e:
            self.logger.log_exception(e)
            raise(e)
